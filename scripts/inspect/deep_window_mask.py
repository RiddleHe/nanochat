"""E1: inference-time sliding-window masking of DEEP layers only, on a model
trained with full attention (Qwen3-8B-Base).
...
Usage:
  python -m scripts.inspect.deep_window_mask --hf-model Qwen/Qwen3-8B-Base
...
def main():
    ...
    ap.add_argument("--out", default="results/deep_window")
    args = ap.parse_args()
    ...
    with open(os.path.join(args.out, "deep_window_mask.json"), "w") as f:
    ...
    fig.tight_layout()
    p = os.path.join(args.out, "deep_window_mask.png")
