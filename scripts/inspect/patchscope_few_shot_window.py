"""Windowed target-layer patchscope using the regular few-shot target prompt."""
import argparse
import csv
import os
import sys
...
def patched_generate_window(adapter, tgt_ids, target_window, tgt_pos, src_vec,
    ...
    """Replace the target residual at `x` in every block in target_window."""
    handles = []
...
def run_one_source(adapter, source, target, windows, max_tokens, normalize=False):
...
def plot_entity(folder, ent, frame_name, windows, rows):
    mat = np.zeros((36, len(windows)), dtype=float)
    for row in rows:
...
def main():
    ...
    for ent_key in ENTITIES:
        ...
        rows, target_info = run_one_source(
            adapter, source, args.target, windows, args.max_tokens,
    ...
    for ent_key, rows in all_rows.items():
        png, pdf = plot_entity(out_dir, ent_key, "regular target, 6-layer windows",
                               windows, rows)
