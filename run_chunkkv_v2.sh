...
if ls "$CKPT/arch_d12_gpt_base_chunk_deep_kv_v2_1.5e18"/meta_*.json >/dev/null 2>&1; then log "exists, skip"; else
  ...
  CUDA_VISIBLE_DEVICES=$G /hdd/mh3897/cc/nanochat/.venv/bin/torchrun --standalone --nproc_per_node=$N \
    -m scripts.base_train -- --depth=12 --model-type=gpt_base_chunk_deep_kv_v2 \
    --model-tag=arch_d12_gpt_base_chunk_deep_kv_v2_1.5e18 \
    --target-flops=1.5e18 --device-batch-size=32 --run=dummy
  log "exit=$?"
fi
m=$(ls "$CKPT/arch_d12_gpt_base_chunk_deep_kv_v2_1.5e18"/meta_*.json 2>/dev/null | tail -1)
[ -n "$m" ] && /hdd/mh3897/cc/nanochat/.venv/bin/python -c "import json;d=json.load(open('$m'));print('v2: step',d['step'],'val_bpb',d.get('val_bpb'))" >> "$LOG"
